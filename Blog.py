...
Created by Prabhjot Singh on 2013-02-03.
...
class EntryIndexHandler(PrabhRequestHandler):
    def get(self):
...
class EntryHandler(PrabhRequestHandler):
    def get(self, slug):
...
class EntryDeleteHandler(PrabhRequestHandler):
        @administrator
        def get(self,slug):
...
class PageHandler(PrabhRequestHandler):
    def get(self, slug):
        admin = users.is_current_user_admin()
...
class TagHandler(PrabhRequestHandler):
    def get(self, slug):
...
class FeedHandler(PrabhRequestHandler):
    def get(self):
...
class NewEntryHandler(PrabhRequestHandler):
    @administrator
