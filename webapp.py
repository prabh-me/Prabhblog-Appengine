...
from utils import PrabhRequestHandler, administrator, Config
import blog
import admin


class LoginHandler(PrabhRequestHandler):
    def get(self):
...
class LogoutHandler(PrabhRequestHandler):
    def get(self):
        user = users.get_current_user()
        if user:
...
class HomePageHandler(PrabhRequestHandler):
    def get(self):
...
def main():
    ...
    if config.count() > 0:
        ...
        config1 = Config(title="Prabh Blog")
        config1.put()
